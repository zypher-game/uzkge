...
impl VerifierParams {
    ...
    pub fn split(
        self,
    ) -> Result<(VerifierParamsSplitCommon, VerifierParamsSplitSpecific), UzkgeError> {
        Ok((
...
pub fn load_srs_params(size: usize) -> Result<KZGCommitmentSchemeBN254, UzkgeError> {
    let srs = SRS.ok_or(UzkgeError::MissingSRSError)?;
