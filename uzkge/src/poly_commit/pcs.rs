...
use crate::{
    errors::UzkgeError, poly_commit::field_polynomial::FpPolynomial, utils::transcript::Transcript,
};
...
pub trait PolyComScheme: Sized + Eq + PartialEq + Clone {
    ...
    ) -> Result<Self::Commitment, UzkgeError>;
    ...
    fn batch_prove(
        ...
        if !rem.is_zero() {
            return Err(UzkgeError::PCSProveEvalError);
        }
        ...
        if let Some(lagrange_pcs) = lagrange_pcs {
            ...
            let q_eval =
                FpPolynomial::fft(&sub_q, max_power_of_2).ok_or(UzkgeError::PCSProveEvalError)?;
    ...
    fn shrink_to_verifier_only(&self) -> Result<Self, UzkgeError>;
}
