...
contract PlonkVerifier {
    ...
    function verify_proof(address vk1, address vk2, bool shuffle_specified) private view returns (bool) {
        assembly {
            ...
            function load_pi_indice(sig, v, i) -> result {
            ...
            // 3. compute PI(\zeta).
            {
                ...
                let root_pow := load_pi_indice(sig1, vk1, sub(pi_length, 1))
                let denominator := addmod(zeta, sub(r, root_pow), r)
                mstore(end_ptr, denominator)
                ...
                for {
                    ...
                    let lagrange_constant := load_pi_indice(sig2, vk2, i)
            ...
            // 5. derive the linearization polynomial commitment.
            {
...

                {
                    ...
                    scalar_mul(mload(CM_Q7_X_LOC), mload(CM_Q7_Y_LOC), sub(r, wo))
                    point_add_in_memory(r_commitment_x, r_commitment_y)
                    r_commitment_x := mload(mload(0x40))
                    r_commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_SHUFFLE_PUBLIC_KEY_8_X_LOC), mload(CM_SHUFFLE_PUBLIC_KEY_8_Y_LOC), tmp_scalar_0)
                    ...
                    scalar_mul(mload(CM_SHUFFLE_PUBLIC_KEY_4_X_LOC), mload(CM_SHUFFLE_PUBLIC_KEY_4_Y_LOC), tmp_scalar_1)
                    ...
                    scalar_mul(commitment_x, commitment_y, mload(SEL_00_LOC))
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))

                    {
                        ...
                        point_add_in_memory(tmp_commitment_x, tmp_commitment_y)
                        ...
                        scalar_mul(tmp_commitment_x, tmp_commitment_y, mload(SEL_01_LOC))
                        point_add_in_memory(commitment_x, commitment_y)
                        commitment_x := mload(mload(0x40))
                        commitment_y := mload(add(mload(0x40), 0x20))
                    }

                    {
                        ...
                        scalar_mul(tmp_commitment_x, tmp_commitment_y, mload(SEL_10_LOC))
                        point_add_in_memory(commitment_x, commitment_y)
                        commitment_x := mload(mload(0x40))
                        commitment_y := mload(add(mload(0x40), 0x20))
                    }

                    {
                        ...
                        scalar_mul(tmp_commitment_x, tmp_commitment_y, mload(SEL_11_LOC))
                        point_add_in_memory(commitment_x, commitment_y)
                        commitment_x := mload(mload(0x40))
                        commitment_y := mload(add(mload(0x40), 0x20))
                    }

                    scalar_mul(commitment_x, commitment_y, mload(ALPHA_POW_10_LOC))
                    point_add_in_memory(r_commitment_x, r_commitment_y)
                    r_commitment_x := mload(mload(0x40))
                    r_commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_SHUFFLE_PUBLIC_KEY_0_X_LOC), mload(CM_SHUFFLE_PUBLIC_KEY_0_Y_LOC), tmp_scalar_1)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(mload(CM_SHUFFLE_PUBLIC_KEY_4_X_LOC), mload(CM_SHUFFLE_PUBLIC_KEY_4_Y_LOC), tmp_scalar_2)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(commitment_x, commitment_y, mload(ALPHA_POW_11_LOC))
                    point_add_in_memory(r_commitment_x, r_commitment_y)
                    r_commitment_x := mload(mload(0x40))
                    r_commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_SHUFFLE_GENERATOR_8_X_LOC), mload(CM_SHUFFLE_GENERATOR_8_Y_LOC), tmp_scalar_0)
                    let commitment_x := mload(mload(0x40))
                    let commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(mload(CM_SHUFFLE_GENERATOR_4_X_LOC), mload(CM_SHUFFLE_GENERATOR_4_Y_LOC), tmp_scalar_1)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(commitment_x, commitment_y, mload(ALPHA_POW_12_LOC))
                    point_add_in_memory(r_commitment_x, r_commitment_y)
                    r_commitment_x := mload(mload(0x40))
                    r_commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_SHUFFLE_GENERATOR_0_X_LOC), mload(CM_SHUFFLE_GENERATOR_0_Y_LOC), tmp_scalar_1)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(mload(CM_SHUFFLE_GENERATOR_4_X_LOC), mload(CM_SHUFFLE_GENERATOR_4_Y_LOC), tmp_scalar_2)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                    ...
                    scalar_mul(commitment_x, commitment_y, mload(ALPHA_POW_13_LOC))
                    point_add_in_memory(r_commitment_x, r_commitment_y)
                    r_commitment_x := mload(mload(0x40))
                    r_commitment_y := mload(add(mload(0x40), 0x20))
                }
            ...
            // 6. Combine multiple commitments(opening in zeta) into one commitment.
            {
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_Q_ECC_X_LOC), mload(CM_Q_ECC_Y_LOC), multiplier)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_W0_SEL_X_LOC), mload(CM_W0_SEL_Y_LOC), multiplier)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_W1_SEL_X_LOC), mload(CM_W1_SEL_Y_LOC), multiplier)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                }
                ...
                if shuffle_specified {
                    ...
                    scalar_mul(mload(CM_W2_SEL_X_LOC), mload(CM_W2_SEL_Y_LOC), multiplier)
                    point_add_in_memory(commitment_x, commitment_y)
                    commitment_x := mload(mload(0x40))
                    commitment_y := mload(add(mload(0x40), 0x20))
                }
